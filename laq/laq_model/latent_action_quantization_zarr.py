        ...
        *_, h, w, _ = shape
        ...
        if ((step % 10 == 0 and step < 100)  or (step % 100 == 0 and step < 1000) or (step % 500 == 0 and step < 5000)) and step != 0:
            ...
            if dist.is_available() and dist.is_initialized():
                ...
                # 8张卡的code使用次数求和
                dist.all_reduce(
                    self.vq.codebooks_used,
                    op=dist.ReduceOp.SUM
                )

                # 只让rank0执行replacement
                if dist.get_rank() == 0:
                    print(f"update codebook {step}")

                    self.vq.replace_unused_codebooks(
                        tokens.shape[0] * world_size
                    )

                # 将rank0的新codebook同步给全部GPU
                with torch.no_grad():
                    dist.broadcast(
                        self.vq.codebooks.data,
                        src=0
                    )
                ...
                # # 临时检查8张卡的codebook是否完全一致
                # gathered = [
                #     torch.empty_like(self.vq.codebooks)
                #     for _ in range(world_size)
                # ]

                # dist.all_gather(
                #     gathered,
                #     self.vq.codebooks.detach()
                # )

                # if dist.get_rank() == 0:
                #     diffs = [
                #         (x - gathered[0]).abs().max().item()
                #         for x in gathered
                #     ]

                #     print(
                #         f"[CODEBOOK_SYNC_CHECK] "
                #         f"step={step} "
                #         f"max_diff_vs_rank0={diffs}",
                #         flush=True
                #     )
